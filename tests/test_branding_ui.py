...
_BRANDING = ("ovllm-branding-extension",)


def test_branding_extension_injects_favicon_and_header_asset_once():
    install_branding_extension()
    html = '<html><head></head><body><div class="logo-icon"><svg></svg></div></body></html>'
    branded = ui_registry.render_only(html, _BRANDING)
    ...
    assert ui_registry.render_only(branded, _BRANDING) == branded
