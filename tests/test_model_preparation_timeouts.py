...
def test_slow_active_conversion_is_not_timed_out() -> None:
    async def scenario() -> None:
        manager = FakeManager(
            PreparationTimeouts(
                download_stall_seconds=0.03,
                conversion_stall_seconds=0.03,
                finalization_stall_seconds=0.03,
                loading_seconds=0.03,
...
def test_genuinely_stalled_conversion_is_cancelled_and_classified() -> None:
    async def scenario() -> None:
        manager = FakeManager(
            PreparationTimeouts(
                download_stall_seconds=0.025,
                conversion_stall_seconds=0.025,
                finalization_stall_seconds=0.025,
                loading_seconds=1.0,
